...
int Dup(int in, int out, char** cmds, int j);
int Pipe(char** cmds,int* pipee );
...
int leAnaliseComando(char**** cmds, int* n, int* pipe){
    ...
    //conta a quantidade de pipes e verifica &.
...
int Dup(int entrada, int saida, char** cmds, int j){
    pid_t pid;
    if ((pid = fork()) == 0){
        if (entrada != 0){
            dup2(entrada, 0);
...
int Pipe(char** cmds,int* pipee){
    ...
    if(pid < 0){
        ...
        for (int i = 0; i < pipee - 1; i++){ //processo filho
            pipe(fd);
            Dup(entrada, fd[1], cmds, j);
            close(fd[1]);
            entrada = fd[0];
            while (cmds[j] != NULL)
                j++;
